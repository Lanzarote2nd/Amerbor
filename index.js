...
function makeBotInstance(cfg) {
  ...
  function connect() {
    ...
    // ── X-ray detection ───────────────────────────────────────────────────────
    // Two methods used together for high confidence:
    //   1. Hidden-ore approach: player mines directly to a diamond/emerald that
    //      had ZERO exposed faces — impossible to see without X-ray.
    //   2. Ore-rate: ores make up 50%+ of all blocks broken (statistically impossible).
    // Trusted players are never monitored. After a kick, player gets a 2-min grace
    // period before monitoring resumes.

    const SAFE_PLAYERS = new Set(['absi', 'blackend12', 'maytham11', '7amasa']);
    ...
    // Returns true if the ore at orePos was completely hidden before brokenPos was mined
    function wasHiddenOre(orePos, brokenPos) {
      try {
        for (const [dx, dy, dz] of FACE_OFFSETS) {
          const neighbor = orePos.offset(dx, dy, dz);
          if (neighbor.equals(brokenPos)) continue; // this face was solid (just broken)
          const b = bot.blockAt(neighbor);
          if (!b || b.type === 0) return false; // exposed face — visible naturally
        }
        return true; // all 6 faces were solid — completely hidden
      } catch (_) { return false; }
    }

    function kickXray(name, reason) {
      xrayData[name].flagged = true;
      xrayData[name].kickedAt = Date.now();
      console.log(`${tag} [XRAY] Kicking ${name} — ${reason}`);
      try { bot.chat(`/kick ${name} Xray detected: ${reason}`); } catch (e) {
        console.log(`${tag} [XRAY] Kick failed: ${e.message}`);
      }
    }

    // { username: { total, ores, valuableOres, hiddenOreHits, flagged, kickedAt } }
    const xrayData = {};

    function getStats(name) {
      if (!xrayData[name]) xrayData[name] = { total: 0, ores: 0, valuableOres: 0, hiddenOreHits: 0, flagged: false, kickedAt: 0 };
      return xrayData[name];
    }

    // When a player leaves, clear their mining stats but keep kick timestamp
    bot.on('playerLeft', (entity) => {
      if (!entity || !entity.username) return;
      const n = entity.username;
      if (xrayData[n]) {
        xrayData[n].total = 0; xrayData[n].ores = 0;
        xrayData[n].valuableOres = 0; xrayData[n].hiddenOreHits = 0;
        xrayData[n].flagged = false;
        // kickedAt stays so the 2-min cooldown survives a rejoin
      }
    });

    bot.on('blockUpdate', (oldBlock, newBlock) => {
      if (!isConnected || !oldBlock || !newBlock) return;
      if (newBlock.type !== 0 || oldBlock.type === 0) return; // only block breaks

      const brokenPos  = oldBlock.position;
      const brokenName = oldBlock.name;

      // Find closest non-safe player within 6 blocks
      let closest = null, closestDist = 6;
      for (const [name, player] of Object.entries(bot.players || {})) {
        if (name === cfg.name) continue;          // skip the bot itself
        if (SAFE_PLAYERS.has(name)) continue;     // skip trusted players
        const e = player.entity;
        if (!e || !e.position) continue;
        const d = e.position.distanceTo(brokenPos);
        if (d < closestDist) { closestDist = d; closest = name; }
      }
      if (!closest) return;

      const s = getStats(closest);
      if (s.flagged) return;

      // 2-minute cooldown after a kick — give them a fresh start
      if (s.kickedAt && (Date.now() - s.kickedAt) < 2 * 60_000) return;

      s.total++;

      // ── Method 1: hidden-ore approach ────────────────────────────────────
      // Check every neighbor of the broken block for a valuable ore that had
      // no exposed faces before this break (completely buried = only X-ray sees it)
      for (const [dx, dy, dz] of FACE_OFFSETS) {
        const neighborPos = brokenPos.offset(dx, dy, dz);
        const neighborBlock = bot.blockAt(neighborPos);
        if (!neighborBlock || !VALUABLE_ORES.has(neighborBlock.name)) continue;

        if (wasHiddenOre(neighborPos, brokenPos)) {
          s.hiddenOreHits++;
          console.log(`${tag} [XRAY] ${closest} mined directly to hidden ${neighborBlock.name} (hit #${s.hiddenOreHits})`);

          // 2 confirmed hidden-ore approaches = near-certain X-ray
          if (s.hiddenOreHits >= 2) {
            kickXray(closest, `mined to ${s.hiddenOreHits} completely hidden ores`);
            return;
          }
        }
      }

      // ── Method 2: ore-rate (backup) ───────────────────────────────────────
      if (TRACKED_ORES.has(brokenName)) {
        s.ores++;
        if (VALUABLE_ORES.has(brokenName)) s.valuableOres++;
      }

      const oreRate = s.total > 0 ? s.ores / s.total : 0;
      if (s.total >= 30 && s.valuableOres >= 8 && oreRate >= 0.5) {
        kickXray(closest, `${s.valuableOres} rare ores, ${Math.round(oreRate * 100)}% ore rate`);
      }
    });
